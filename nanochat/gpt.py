...
class DynamicBasisQueryAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        # The Triton kernel is registered via torch.library.custom_op (see
        # nanochat/bqa_dyn_triton.py) so inductor sees it as an opaque op and
        # keeps its FX graph contiguous around the call. No graph-break needed
        # — torch.compile fuses the surrounding alpha-softmax / rotary / norm /
        # projections cleanly with the compiled FFN/embed/head.
